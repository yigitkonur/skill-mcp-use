...
server.tool({
    ...
    // ctx provides: ctx.log, ctx.elicit(), ctx.sample(), ctx.reportProgress()
...
|---|---|---|
| `text(string)` | `{ content: [{ type: 'text', text }] }` | Plain text responses |
| `object(data)` | `{ content: [{ type: 'text', text: JSON.stringify(data) }] }` | Structured data |
| `markdown(string)` | `{ content: [{ type: 'text', text }] }` | Markdown content |
| `image(data, mimeType)` | `{ content: [{ type: 'image', data, mimeType }] }` | Base64 images |
| `audio(data, mimeType)` | `{ content: [{ type: 'audio', data, mimeType }] }` | Base64 audio |
| `binary(data, mimeType)` | Binary content block | Raw binary data |
| `file(path)` | File content | File contents |
| `error(message)` | `{ content: [{ type: 'text', text }], isError: true }` | Error responses |
| `mix(...)` | Combined content blocks | Multi-part responses |
| `resource(uri, content)` | Embedded resource content | Resource references |
| `widget(props)` | Structured + UI content | Interactive widgets |
...
server.tool({ name: 'smart-tool', schema: z.object({...}) },
    async (args, ctx) => {
        ...
        ctx.log.debug('Args received', args)
        
        // Progress reporting
        ctx.reportProgress(50, 100)  // 50% done
        ...
        const summary = await ctx.sample('Summarize this data...')
