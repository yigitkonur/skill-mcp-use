...
server.resourceTemplate({
    ...
    name: 'User Profile',
    mimeType: 'application/json',
}, async ({ userId }) => object(await getUser(userId)))
```
...
// Or notify all sessions that the resource list changed:
await sendResourcesListChanged(sessions)
```

```python
# mcp-use Python client — subscribe to resource changes
# (handled automatically by MCPAgent or manually via session)
```
...
| Subscriptions | Manual notification | `notifyResourceUpdated()` | Via session |
